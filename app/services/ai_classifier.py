...
_CLASSIFY_VERSION = "exec_v6_logged"
_SUMMARY_VERSION  = "summary_v10_logged"
_REPLY_VERSION    = "reply_v5_logged"
...
Content: {snippet}"""
        ...
        if response.status_code != 200:
            print(f"❌ CLASSIFY API ERROR: Status {response.status_code} | {response.text}")
            raise Exception(f"AI returned {response.status_code}")

        data = response.json()
        candidates = data.get("candidates", [])
        if not candidates:
            print(f"⚠️ NO CLASSIFY CANDIDATES: {data}")
            raise Exception("AI returned no candidates")
...
Write the grounded summary now: one or two complete sentences only, no preamble (only from the lines above)."""
        ...
        if response.status_code != 200:
            ...
            return f"Error: AI returned {response.status_code}"

        data = response.json()
        candidates = data.get("candidates", [])
        if not candidates:
            print(f"⚠️ NO SUMMARY CANDIDATES: {data}")
            return "Summary unavailable (AI blocked content)"
