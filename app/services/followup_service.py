...
log = get_logger("followup_service", service_name="FollowUpService")
...
class FollowUpService:
    ...
    def list_followups(self, account_email: str) -> List[Dict[str, Any]]:
        with _LOCK:
    ...
    def dismiss_thread(self, account_email: str, thread_id: str) -> None:
    ...
    def rebuild_for_account(self, email_address: str, creds: Dict[str, Any]) -> int:
        ...
        for tid, em in thread_latest.items():
            if tid in dismissed_set:
                continue
            try:
                pr = self._classification.calculate_priority({**em, "account_email": email_address})
            except Exception as e:
...
def list_items(account_email: str) -> List[Dict[str, Any]]:
    ...
    return get_container().followup_service.list_followups(account_email)


def dismiss_thread(account_email: str, thread_id: str) -> None:
    from app.container import get_container

    get_container().followup_service.dismiss_thread(account_email, thread_id)


def rebuild_for_account(email_address: str, creds: Dict[str, Any]) -> int:
    from app.container import get_container

    return get_container().followup_service.rebuild_for_account(email_address, creds)
